...
disp('Plane Wave Decomposition of specified Wavefield');
...
elseif strcmp(params.sampling.mode,'continuous')
  ...
  switch source.type
    case 'plane'
      for n=1:Nsht+1
        progress_bar(n,Nsht+1);  % helper function of SFS-Toolbox
  ...
  for m=1:Npw
    progress_bar(m,Npw);  % helper function of SFS-Toolbox
    for n=1:Nsht+1
...
elseif (strcmp(params.beamformer,'MB'))  
  ...
  switch params.sampling.mode
    case 'continuous'
      [Pnm, kr] = sofia_wgc(Nsht, R, ac, fs, Nfft, phii, thetai, ...
        0, c, wavetype, ri);  % ri will be ignored
    case 'discrete'
