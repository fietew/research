...
%----------------------------------------------
% Define colormap 
CC=[colormap('Summer');flipud(colormap('Autumn'))];
vr=sqrt(u.^2+v.^2);
vr_orig = sqrt(u_orig.^2+v_orig.^2);
vrn_c=round(vr_orig/180*size(CC,1));
ax = newplot;
...
vrn1= [vrn_c;repmat(NaN,size(u));repmat(NaN,size(u))];
