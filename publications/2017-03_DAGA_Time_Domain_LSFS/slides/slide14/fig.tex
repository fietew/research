...
% optional pre-filter
\node[block, draw=darkgreen, below=0.5cm of signal] (prefilter) 
  {\color{darkgreen} $\hat h_{\mathrm{pre}}[n]$};

% delayline
\node[block, right=0.5cm of prefilter, minimum width=6cm] 
  (delayline) {(fractional) delayline};
\foreach \idx/\ratio in {0/0.700, 1/0.500, 2/0.300, N_l/0.100}
{
...
\foreach \idx/\odx/\mdx in {0/0/0,1/1/1, 2/2/0, N_l/4/1}
{
  ...
  \node[above=0.3cm of delayline_in\idx, yshift=0.75*\mdx*\yoffset cm] 
    (weightdelay\odx) {%
      \scriptsize 
      $(\textcolor{red}{w},\textcolor{blue}{\tau})
        (\sfpossec^{(\idx)},\sfposfs)$
    };
  % contributions from other sound sources
  \node[op] (add\idx) at
    ($(delayline.east) + (0.5,-1.0) + (\odx*\xoffset, -\odx*\yoffset)$) {+};
  % sum over all focussed sources
  \node[rectangle, draw, left=0.3cm of add\idx, xshift=-\odx*\xoffset cm] 
  (sum\idx)
    {\footnotesize $\sum$};
  % loudspeakers
  \node[loudspeaker,right=1.0cm of add\idx, anchor=west, xshift=-\odx*\xoffset 
    cm] (driving\idx) {};
...
\foreach \idx/\odx in {0/0,1/1, 2/2, N_l/4}
{
  ...
  % loudspeakers
  \draw[connect] (add\idx) -- (driving\idx);
}

% dots
\foreach \label in {sum,driving}
{
