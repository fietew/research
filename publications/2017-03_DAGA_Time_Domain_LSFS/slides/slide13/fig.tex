...
\node[block, minimum width=2cm, minimum height=4cm] (wfs1) {WFS};
\node[block, minimum width=2cm, minimum height=4cm, right=2cm of wfs1] (wfs2) 
{WFS};

\foreach \idx/\ratio in {0/0.875, 1/0.625, N_s/0.125}
{
...
\foreach \idx/\ratio in {0/0.900, 1/0.700, 2/0.500, tmp/0.300, N_{fs}/0.100}
{
  \pgfmathsetmacro{\ratioinv}{1-\ratio}
...
\foreach \idx/\ratio in {0/0.900, 1/0.700, 2/0.500, N_l/0.100}
{
  \pgfmathsetmacro{\ratioinv}{1-\ratio}
...
\path (driving2) -- node[pos=0.5]{\huge$\vdots$} (drivingN_l);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Connections

\foreach \idx in {0,1,N_s}{
  \draw[connect] (source\idx) -- (wfs1_in\idx);
}

\foreach \idx/\ratio in {0, 1, 2, N_{fs}}
{  
  \draw[connect] (wfs1_out\idx) -- (wfs2_in\idx) node[above, pos=0.5]
    {\scriptsize $\sfdriving[prefix=time,subscript=fs][\sfposfs^{(\idx)}, n]$};
}

\foreach \idx in {0,1, 2, N_l}
{
  % loudspeakers
