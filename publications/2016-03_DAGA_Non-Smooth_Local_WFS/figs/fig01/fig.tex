...
\draw[|-|] (-\L/2,-\L/2) -- (\L/2,-\L/2) node[near end, above]{$L$};
% boundary and area
\node[left] at (-\R/2,-\R/2) {$\sfset$};
\draw[latex-] (speaker9012.cone) -- +(-45:0.5) node[right, at end] 
  {$\sfboundary$};
  
%%% virtual sound field %%%
\draw (0,0) -- (\phis:\Rs) node[sloped, near end, below] 
  {$\sfcylr[subscript=s]$};
...
\draw[thick, -latex] (0,-0.9*\Rcoord) -- (0,\Rcoord) node[at end, left] {$y$};
