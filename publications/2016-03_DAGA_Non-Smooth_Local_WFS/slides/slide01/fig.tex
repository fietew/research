...
\foreach[count=\sdx] \phi in {0, 90, 180, 270} {
  ...
  % draw numbers
  \pgfmathsetmacro{\number}{(\sdx-1)*\Nseg+1}%
  \node[] at ($(segmentshifted)+(\phi+90:-\L/2)$)%
    {\pgfmathprintnumber{\number}};
  \pgfmathsetmacro{\number}{\sdx*\Nseg}%
  \node at ($(segmentshifted)+(\phi+90:\L/2)$)%
    {\pgfmathprintnumber{\number}};
...
% x0 and n0 label
\node[left]  at (speaker1805.west)  {$\sfpossec$};
\draw[-latex] (speaker1805.cone) -- +(0.3,0) node[right, at end] 
  {$\sfnormalsec$};
% label indicating the lenght of the loudspeaker array
\draw[|-|] (-\L/2,-\L/2) -- (\L/2,-\L/2) node[near end, above]{$L=4$ m};

%%% virtual sound field %%%
\draw[-latex] (0,0) -- (\phis:\Rs) node[sloped, at end, anchor=north east] 
  {\footnotesize $\sfcylr[subscript=s] = 4$ m};
...
%%% coordinate axes %%%
\draw[thick, -latex] (-\Rcoord,0) -- (\Rcoord,0) node[at end, below] 
{$x$};
\draw[thick, -latex] (0,-\Rcoord) -- (0,\Rcoord) node[at end, left] 
{$y$};
