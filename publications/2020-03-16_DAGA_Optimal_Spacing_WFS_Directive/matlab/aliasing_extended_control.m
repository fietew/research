function [f, fx0] = aliasing_extended_control(x0, kSx0, x, minmax_kGt_fun, minmax_kSt_fun, conf)
...
for xdx = 1:size(x,1)
    ...
    fx0(xdx,:) = conf.c ./ (deltax0.*max(abs(kSt-kGtmin),abs(kSt-kGtmax)));
    fx0(xdx,isnan(kGtmin)) = Inf;
end
f = min(fx0, [], 2);  % ignores nan