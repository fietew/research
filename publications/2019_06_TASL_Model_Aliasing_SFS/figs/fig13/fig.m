...
%% Aliasing-Free Circle by Wierstorf
[~, rfs] = cart2pol(xs(1),xs(2));

for f = fvec
  Ral = (1 - rfs/R)*L*conf.c./(2.*pi.*f);
  
  gp_save(sprintf('Ral_f%d.txt',f), [xs(1), xs(2), Ral]);
end

%% Comparison of Models
xex = [0,0,0];

fSnew = aliasing(x0,kSx0,xex,conf)
fSold = L*conf.c./(2.*pi.*R)*(R - rfs)./vector_norm(xex - xs(1:3),2)
...
for f = fvec
  ...
  % normalisaton factor
  g = sound_field_mono(xref(1), xref(2), xref(3), [xs(1:3), 0, 1, 0, 1], srcg, 1, f, conf);
